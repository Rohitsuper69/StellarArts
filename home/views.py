...
def product_detail_view(request, handle=None):
    ...
    attachments = ProductAttachment.objects.filter(product=object)
    is_owner=False
    if request.user.is_authenticated:
        is_owner = True
    context={"object":object, "is_owner":is_owner,"attachments":attachments}
...
def product_attachment_download_view(request, handle=None, pk=None):
    ...
    can_download = attachment.isFree 
    if request.user.is_authenticated:
        can_download=True
